...
async function migrateChecklistDocuments() {
  try {
    console.log('Starting checklist documents migration...');
    ...
    // Update existing permit_checklists to set documents_required based on checklist requirements
    await sequelize.query(`
      UPDATE permit_checklists 
      SET documents_required = TRUE 
      WHERE checklist_id IN (
        SELECT id FROM checklists 
        WHERE category IN ('structural', 'electrical', 'plumbing', 'mechanical', 'environmental', 'zoning')
      );
    `);

    console.log('✓ Updated existing permit_checklists with document requirements');
...
if (require.main === module) {
  migrateChecklistDocuments();
}

module.exports = migrateChecklistDocuments;
